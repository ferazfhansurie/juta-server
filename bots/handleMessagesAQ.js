...
async function handleNewMessagesAQ(req, res) {
    try {
        console.log('Handling new messages from AQ...');
        ...
        for (const message of receivedMessages) {
            ...
            if (contactPresent !== null) {
                ...
                if(stopTag.includes('stop bot')){
                    ...
                    const threadIdField = contactPresent.customFields.find(field => field.id === 'MWNYRbxlFBaI3mNlg8mQ');
                    if (threadIdField) {
...
async function handleOpenAINameAssistant(senderName) {
    ...
    const assistantId = 'asst_Wl7sZC2wyLgPXEjfhZDLMYsQ';
...
async function checkingStatus(threadId, runId) {
    ...
    if(status == 'completed') {
        ...
        try{
            ...
            console.log("error from handleNewMessagesAQ: "+error)
        }
...
async function fetchConfigFromDatabase() {
    try {
        const docRef = db.collection('companies').doc('015');
...
module.exports = { handleNewMessagesAQ };