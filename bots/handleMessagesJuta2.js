...
async function handleNewMessagesJuta2(client, msg, botName, phoneIndex) {
    ...
    try {
        ...
        // Check if there's an existing buffer for this sender
        if (messageBuffer.has(extractedNumber)) {
            // Add this message to the buffer
            messageBuffer.get(extractedNumber).messages.push(msg);
            return; // Exit the function, we'll process later
        }

        // Create a new buffer for this sender
        messageBuffer.set(extractedNumber, {
            messages: [msg],
            timeoutId: setTimeout(() => processBufferedMessages(client, extractedNumber, botName, phoneIndex), BUFFER_TIMEOUT)
        });
...
async function processBufferedMessages(client, extractedNumber, botName, phoneIndex) {
    const buffer = messageBuffer.get(extractedNumber);
    if (!buffer) return;

    messageBuffer.delete(extractedNumber);
    ...
    // Now process the combined message
    await processMessage(client, combinedMsg, botName, phoneIndex);
}

async function processMessage(client, msg, botName, phoneIndex) {
    // This function will contain most of the original handleNewMessagesJuta2 logic
    const idSubstring = botName;
    const sender = {
        to: msg.from,
        name: msg.notifyName,
