...
class msuSpreadsheetLeads {
  ...
  async processRow(row, rowIndex) {
    ...
    // Check if phone number exists in the current sheet or the other sheet
    const isDuplicate = await this.checkPhoneNumberDuplicate(phoneNumber);
    if (isDuplicate) {
      console.log(`Phone number ${phoneNumber} is a duplicate. Marking as duplicate.`);
    ...
    - This is the field of study of the user and only this :
    Field Of Study : ${fieldOfStudy}

    - If the user asks for a brochure, you will only provide the brochure of user's first field of study that was assigned to the user.

    - Use only the ### Brochure Flow not the ### Conditional Offer Letter FLOW for this user
    - Use only the ### Brochure Flow not the ### Conditional Offer Letter FLOW for this user
    - Use only the ### Brochure Flow not the ### Conditional Offer Letter FLOW for this user
    - Use only the ### Brochure Flow not the ### Conditional Offer Letter FLOW for this user
  `;
...
module.exports = msuSpreadsheetLeads;